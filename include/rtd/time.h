...
A timer is stopped, whose status is `deleted`, and waiting the poller to remove
it from heap.
...
SysTimePoint Now() { return system_clock::now(); }
...
// The status of timer
enum class _TimerStatus { deleted, removed, waiting, running, noStatus };
...
struct _TimersHeap {
  std::priority_queue<_SharedTimer, std::vector<_SharedTimer>,
                      _SharedTimerComparsion>
      timers;
  std::mutex mu;
  ...
  const _SharedTimer &Top() { return timers.top(); }
...
void _BadTimer() { throw std::logic_error("racy use of timers"); }

// Run a timer.
// Pop a timer and call Do(). Calculate the next `when` if the timer is a
// ticker, and push into heap again. Pop it, call Do() and End() if it is a
// disposable timer.
  ...
  while (1) {
    ...
    if (t->status == _TimerStatus::waiting) {
      ...
      break;
               ...
               t->status == _TimerStatus::removed) { // An inactive timer should
                                                     // be popped
      _BadTimer();
  ...
  while (1) {
    if (t->status == _TimerStatus::waiting) {
      ...
      continue; // try again later
               ...
               _TimerStatus::noStatus) { // do not start or end of run
      t->status = _TimerStatus::removed;
      return true;
...
template <typename T, typename U> SysTimePoint When(duration<T, U> d) {
  return Now() + d;
}

// The user interface of timer.
template <typename T, typename U> class Timer {
public:
  explicit Timer(duration<T, U> period)
      : period_(period), t_(std::make_shared<_Timer>()) {
  ...
  Timer &Start() {
    if (t_->status != _TimerStatus::noStatus) {
      throw std::logic_error(
          "cannot start timer that has been started or stopped.");
    }
    _Set();
    _AddTimer(t_);
    return *this;
  ...
  SharedChan<SysTimePoint> Channel() { return c_; }
...
// A ticker.
template <typename T, typename U> class Ticker : public Timer<T, U> {
